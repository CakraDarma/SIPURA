	...
	return (
		<div className='flex gap-6 md:gap-10'>
			<Link href='/' className='items-center hidden space-x-2 md:flex'>
				...
				<span className='hidden text-lg font-semibold text-black sm:inline-block'>
					{siteConfig.name}
				</span>
			</Link>
			{items?.length ? (
				<nav className='hidden gap-6 md:flex'>
					{items?.map((item, index) => (
						<Link
							...
							className={cn(
								'flex items-center text-lg font-medium hover:border-b-2 border-foreground sm:text-base',
